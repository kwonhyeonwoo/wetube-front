import { useForm, type SubmitHandler } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod/src/zod.js";
import { useCallback } from "react";
import { videoSchema, type VideoType } from "@/schema/media.schema";
import { useToastStore } from "@/store/useToastStore";
import VideoFormTemplate from "@/pages/video/components/VideoFormTemplate/VideoFormTemplate";
import { useMediaPreview } from "@/hooks/useMediaPreview";
import { useUidStore } from "@/store/useUserStore";
import { useWriteVideoMutation } from "@/hooks/mutations/video/useWirteVideoMutation";

const VideoWriteContainer = () => {
  const uid = useUidStore();
  const { mutate } = useWriteVideoMutation(uid ?? "");
  const addToast = useToastStore((state) => state.addToast);
  const { register, handleSubmit, watch, setValue } = useForm<VideoType>({
    resolver: zodResolver(videoSchema),
    defaultValues: {
      title: "",
      content: "",
      categories: "movie",
      hashtags: [],
    },
  });
  const { mediaPreview, addPreviewMedia } = useMediaPreview({
    name: "video",
    setValue,
  });
  const onSubmit: SubmitHandler<VideoType> = useCallback(
    (data: VideoType) => {
      mutate({
        video: data.video,
        title: data.title,
        content: data.content,
        categories: data.categories,
        hashtags: data.hashtags,
      });
    },
    [mutate],
  );
  const onInvalid = (err: any) => {
    const firsrtError = Object.values(err)[0];
    addToast(firsrtError as string);
  };
  const currentCategory = watch("categories");
  const currentTags = watch("hashtags");
  return (
    <VideoFormTemplate
      currentCategory={currentCategory}
      currentTags={currentTags}
      videoPreview={mediaPreview}
      addPreviewMedia={addPreviewMedia}
      register={register}
      setValue={setValue}
      onInvalid={onInvalid}
      onSubmit={onSubmit}
      handleSubmit={handleSubmit}
    />
  );
};

export default VideoWriteContainer;
